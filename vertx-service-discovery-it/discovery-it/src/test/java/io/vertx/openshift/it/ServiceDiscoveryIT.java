...
public class ServiceDiscoveryIT extends AbstractKubernetesIT {
  ...
  private void initializeServiceAccount() {
    OpenshiftHelper.oc_execute("policy", "add-role-to-user", "view", "admin", "-n", client.getNamespace());
    OpenshiftHelper.oc_execute("policy", "add-role-to-group", "view",
  ...
  public void testServerSideRouting() throws Exception {
    ...
    await().atMost(3, TimeUnit.MINUTES).until(() -> client.pods().list().getItems().size() == 3);
